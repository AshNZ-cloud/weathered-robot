...
// ---------- Ground ----------
const groundMat = new THREE.MeshStandardMaterial({ color: 0x1c1f26, roughness: 0.95, metalness: 0.05 });
const ground = new THREE.Mesh(new THREE.CircleGeometry(30, 48), groundMat);
ground.name = 'ground';
...
function makeArmorTexture(baseHex, scratchDensity = 26) {
  const size = 512;
...
function makeHullTexture(baseHex, stripeHex) {
  const size = 512;
  const canvas = document.createElement('canvas');
  canvas.width = size; canvas.height = size;
  const ctx = canvas.getContext('2d');
...
const darkMetal = new THREE.MeshStandardMaterial({ map: armorTexDark, roughness: 0.55, metalness: 0.8 });
const midMetal = new THREE.MeshStandardMaterial({ map: armorTexMid, roughness: 0.5, metalness: 0.85 });
const jointMetal = new THREE.MeshStandardMaterial({ color: 0x20242c, roughness: 0.6, metalness: 0.7 });
const visorMat = new THREE.MeshStandardMaterial({ color: 0x2255ff, emissive: 0x1144ff, emissiveIntensity: 1.4, roughness: 0.2, metalness: 0.2 });
const goldTrim = new THREE.MeshStandardMaterial({ color: 0x8a7748, roughness: 0.4, metalness: 0.9 });

// Torso
const torso = new THREE.Mesh(new THREE.BoxGeometry(1.5, 1.9, 0.9), darkMetal);
...
// Chest plate detail
const chestPlate = new THREE.Mesh(new THREE.BoxGeometry(1.1, 1.0, 0.15), midMetal);
chestPlate.name = 'chestPlate';
chestPlate.position.set(0, 2.55, 0.5);
chestPlate.castShadow = true;
robot.add(chestPlate);

const chestCore = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.1, 20), visorMat);
...
// Waist
const waist = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.65, 0.5, 12), jointMetal);
...
// Hips / pelvis armor
const pelvis = new THREE.Mesh(new THREE.BoxGeometry(1.3, 0.5, 0.8), darkMetal);
...
const helmet = new THREE.Mesh(new THREE.SphereGeometry(0.42, 24, 20, 0, Math.PI * 2, 0, Math.PI * 0.75), midMetal);
helmet.name = 'helmet';
helmet.castShadow = true;
headGroup.add(helmet);

const helmetBack = new THREE.Mesh(new THREE.SphereGeometry(0.43, 20, 16), darkMetal);
...
const visor = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.16, 0.12), visorMat);
visor.name = 'visor';
visor.position.set(0, 0.02, 0.36);
headGroup.add(visor);

const helmetRidge = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.5, 0.46), goldTrim);
helmetRidge.name = 'helmetRidge';
helmetRidge.position.set(0, 0.28, 0.02);
headGroup.add(helmetRidge);

const antenna = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.02, 0.4, 8), jointMetal);
antenna.name = 'antenna';
antenna.position.set(0.15, 0.55, -0.1);
antenna.rotation.z = -0.2;
headGroup.add(antenna);
const antennaTip = new THREE.Mesh(new THREE.SphereGeometry(0.035, 8, 8), new THREE.MeshStandardMaterial({ color: 0xff3355, emissive: 0xff3355, emissiveIntensity: 1.2 }));
...
for (let i = -1; i <= 1; i++) {
  const vent = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.1, 0.06), jointMetal);
...
function makeShoulder(side) {
  ...
  const pauldronOuter = new THREE.Mesh(new THREE.SphereGeometry(0.38, 16, 14, 0, Math.PI * 2, 0, Math.PI * 0.6), midMetal);
  pauldronOuter.name = `pauldronOuter_${side}`;
  pauldronOuter.castShadow = true;
  group.add(pauldronOuter);
  const pauldronRim = new THREE.Mesh(new THREE.TorusGeometry(0.32, 0.04, 8, 16, Math.PI), goldTrim);
...
function makeArm(side) {
  ...
  const upperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.19, 0.17, 0.75, 10), darkMetal);
  upperArm.name = `upperArm_${side}`;
  upperArm.position.y = -0.4;
  upperArm.castShadow = true;
  armGroup.add(upperArm);

  const elbow = new THREE.Mesh(new THREE.SphereGeometry(0.16, 12, 10), jointMetal);
  ...
  const forearm = new THREE.Mesh(new THREE.CylinderGeometry(0.17, 0.15, 0.7, 10), midMetal);
  forearm.name = `forearm_${side}`;
  forearm.position.y = -0.35;
  forearm.castShadow = true;
  forearmGroup.add(forearm);

  // Plasma gun mounted under forearm
  const gunGroup = new THREE.Group();
  gunGroup.name = `plasmaGun_${side}`;
  gunGroup.position.set(0, -0.65, 0.1);
  forearmGroup.add(gunGroup);

  const gunBody = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.22, 0.55), jointMetal);
  gunBody.name = `gunBody_${side}`;
  gunBody.castShadow = true;
  gunGroup.add(gunBody);

  const gunBarrel = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.08, 0.4, 10), darkMetal);
  gunBarrel.name = `gunBarrel_${side}`;
  gunBarrel.rotation.x = Math.PI / 2;
  gunBarrel.position.z = 0.45;
  gunGroup.add(gunBarrel);

  const emitterMat = new THREE.MeshStandardMaterial({ color: 0x66ffee, emissive: 0x33ffdd, emissiveIntensity: 2.2, roughness: 0.3 });
  const emitter = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.075, 0.06, 12), emitterMat);
...
function makeLeg(side) {
  ...
  const thigh = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.19, 0.75, 10), darkMetal);
  thigh.name = `thigh_${side}`;
  thigh.position.y = -0.38;
  thigh.castShadow = true;
  legGroup.add(thigh);

  const knee = new THREE.Mesh(new THREE.SphereGeometry(0.18, 12, 10), jointMetal);
  knee.name = `knee_${side}`;
  knee.position.y = -0.76;
  legGroup.add(knee);

  const shin = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.15, 0.75, 10), midMetal);
  shin.name = `shin_${side}`;
  shin.position.y = -1.15;
  shin.castShadow = true;
  legGroup.add(shin);

  const foot = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.16, 0.5), jointMetal);
...
const hullMat = new THREE.MeshStandardMaterial({ map: hullTex, roughness: 0.45, metalness: 0.6 });
const hullMatGrey = new THREE.MeshStandardMaterial({ color: 0x6b7686, roughness: 0.5, metalness: 0.65 });
const accentBlue = new THREE.MeshStandardMaterial({ color: 0x1c4d8f, roughness: 0.35, metalness: 0.7 });
const cockpitGlass = new THREE.MeshPhysicalMaterial({ color: 0x1a2a44, roughness: 0.05, metalness: 0.1, transmission: 0.6, transparent: true, opacity: 0.85, ior: 1.4 });
const engineMat = new THREE.MeshStandardMaterial({ color: 0x113355, emissive: 0x3388ff, emissiveIntensity: 1.6, roughness: 0.3 });

// Fuselage
const fuselage = new THREE.Mesh(new THREE.CapsuleGeometry(0.55, 3.2, 8, 16), hullMat);
...
// Nose cone
const nose = new THREE.Mesh(new THREE.ConeGeometry(0.5, 1.0, 16), hullMatGrey);
nose.name = 'nose';
...
// Cockpit canopy
const cockpit = new THREE.Mesh(new THREE.SphereGeometry(0.35, 16, 12, 0, Math.PI * 2, 0, Math.PI * 0.6), cockpitGlass);
...
// Straight wings (left/right), simple flat trapezoid slabs
function makeWing(side) {
  ...
  const stripe = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.02, 0.14), accentBlue);
  stripe.name = `wingStripe_${side}`;
  stripe.position.set(sign * 1.1, 0.065, 0);
  wingGroup.add(stripe);

  const thruster = new THREE.Mesh(new THREE.CylinderGeometry(0.14, 0.16, 0.35, 12), engineMat);
...
// Tail fin
const tailFin = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.7, 0.08), hullMatGrey);
...
const tailStripe = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.12, 0.09), accentBlue);
tailStripe.name = 'tailStripe';
tailStripe.position.set(-1.55, 0.55, 0);
ship.add(tailStripe);

// Rear main engines
function makeMainEngine(offsetZ) {
  const eng = new THREE.Group();
  eng.name = `mainEngineGroup_${offsetZ > 0 ? 'R' : 'L'}`;
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.32, 0.7, 14), hullMatGrey);
  body.name = `mainEngineBody_${offsetZ > 0 ? 'R' : 'L'}`;
  body.rotation.z = Math.PI / 2;
  body.castShadow = true;
  eng.add(body);
  const nozzle = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.26, 0.15, 14), engineMat);
...
for (let i = 0; i < 5; i++) {
  const greeble = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.06, 0.4), accentBlue);
...
const boltMat = new THREE.MeshStandardMaterial({ color: 0x66ffee, emissive: 0x33ffdd, emissiveIntensity: 3 });

const MAX_BOLTS = 20;

function fireBolt(fromWorldPos, direction) {
...
function fireBothGuns() {
  const dirL = new THREE.Vector3(0, 0, 1).applyQuaternion(robot.getWorldQuaternion(new THREE.Quaternion()));
...
function animate() {
  ...
  if (qualityCheckTimer >= 2) {
    ...
    if (currentFPS < 30 && qualityLevel > 0) {
      ...
      console.log(`Adaptive quality: down to ${qualityLevel} (${currentFPS} FPS)`);
    } else if (currentFPS > 55 && qualityLevel < 2) {
      applyQuality(qualityLevel + 1);
      console.log(`Adaptive quality: up to ${qualityLevel} (${currentFPS} FPS)`);
    }
  }

  // Robot idle breathing + visor pulse
